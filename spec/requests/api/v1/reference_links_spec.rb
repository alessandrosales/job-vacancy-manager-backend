...
RSpec.describe "API V1 — Reference links", openapi_spec: "v1/swagger.yaml" do
  path "/api/v1/reference-links" do
    get "Lists reference links for the current user (paginated by default)" do
      tags "ReferenceLinks"
      ...
      response 200, "OK" do
        schema "$ref" => "#/components/schemas/paginated_reference_links"
        ...
        run_test! do |response|
          ...
          expect(payload["data"].first["title"]).to eq("Docs")
...
RSpec.describe "API V1 — Reference links pagination", type: :request do
  it_behaves_like "paginated index", path: "/api/v1/reference-links" do
    let!(:owner) do
      User.create!(
        name: "Pag", email: "reflinks-pag@example.com",
    ...
    before do
      3.times { |i| owner.reference_links.create!(title: "T#{i}", url: "https://x#{i}.com") }
    end
  end
end
